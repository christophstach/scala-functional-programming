...
object Section2 {
  def main(args: Array[String]): Unit = {
    ...
    val entriesACount = rdd.filter(row => row.getString(0) == "a").count()
    ...
    /**
      ...
      */
    val entriesToSums = rdd.map(row => row.getString(0) -> BigInt(row.getInt(1))).reduceByKey((acc, curr) => acc + curr).collect
